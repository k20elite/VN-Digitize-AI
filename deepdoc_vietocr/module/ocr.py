...
def _resolve_vietocr_weight_path(filename: str) -> str:
    """Return absolute path to bundled VietOCR weights."""
    weight_path = Path(__file__).resolve().parents[1] / "vietocr" / "weight" / filename
    return str(weight_path)


def _probability_to_score(probability, default: float = 1.0) -> float:
    """Convert predictor probability output into a float score in [0, 1]."""
...
class TextRecognizer:
    def __init__(self, model_dir=None, device_id: int | None = None):
        ...
        config['weights'] = _resolve_vietocr_weight_path("vgg_seq2seq.pth")
    ...
    def __call__(self, img_list):
        ...
        for img in img_list:
            ...
            text, probability = self.detector.predict(img, return_prob=True)
            results.append((text, _probability_to_score(probability)))
        return results, 0.0
